...
// Config holds configuration for creating the Draw Agent.
//
// DrawAgent is a TagentAgent stub for image generation. It is currently
// disabled and will be activated in a future version when an image
// generation model is attached.
type Config struct {
	...
	Prompt PromptConfig // Optional: overrides PromptDir + "draw_agent.md" if set
...
// NewAgent creates a TagentAgent stub for image generation.
func NewAgent(cfg Config) (*agent.TagentAgent, error) {
	if cfg.Model == nil {
		return nil, fmt.Errorf("draw agent: model is required")
	}
	...
	if !cfg.Prompt.IsEmpty() {
		...
		if err != nil {
			return nil, fmt.Errorf("draw agent: load prompt: %w", err)
		}
	} else {
		systemPrompt, err = loader.LoadFromFile("draw_agent.md")
		if err != nil {
			return nil, fmt.Errorf("draw agent: load prompt: %w", err)
		}
	}
	...
	agentCfg := &agent.TagentConfig{
		Name:              "draw",
		Description:       "Image generation agent (stub). Not yet implemented.",
		Model:             cfg.Model,
	...
	if err != nil {
		return nil, fmt.Errorf("draw agent: create tagent agent: %w", err)
	}
