...
func TestNewAgent_Success(t *testing.T) {
	cfg := Config{
		...
		Tools:    []trpctool.Tool{&mockTool{name: "tts"}},
		Prompt:   PromptConfig{Inline: "You are a speak agent."},
	}

	ta, err := NewAgent(cfg)
	require.NoError(t, err)
	require.NotNil(t, ta)
	assert.Equal(t, "speak", ta.Info().Name)

	tools := ta.Tools()
	require.Len(t, tools, 1)
	assert.Equal(t, "tts", tools[0].Declaration().Name)
}
