...
面向长期无人值守运行的平台级能力。除「重启连续」随冷启动自动生效外，其余默认关闭（配置门控，零行为变化），按需开启：

| 能力 | 一句话说明 |
|------|-----------|
| ♻️ **重启连续** | 对话投影 / 任务板 / 常驻会话三层状态全部从事实链重建——重启不丢上下文、逐字节复原、prefix-cache 复用 |
| 🔥 **非重启热更** | 配置结构变更经懒检查自动换执行器：校验失败不切换（旧执行器原样服务）、进行中回合不受影响，支持回滚 |
| 🛡 **治理闸** | 风险四级分级 + 预算滑窗 + critical 异步人工审批 + 审计账本 + 目标登记工具 |
| 🧬 **自进化** | git 原生改进通道：文件即真源 + 版本回滚；后验评估劣化只出建议——执行权永远在 agent |
| 🚡 **常驻可靠性** | 事件总线磁盘溢出（不丢事件）+ 外部依赖退化追踪 + 存储失败兜底重放 |
| 🔁 **回执-反馈闭环** | 任务结算自动生成反馈 + 外部评分接口 + 因果绑定；负反馈率进入自进化判据 |
