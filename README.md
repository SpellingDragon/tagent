...
>
> 下一个回合起我已用新模型、新工具工作——保存时校验通过才切换，失败则旧执行器原样服务；我进行中的回合用旧模型跑完，可回滚 **（非重启热更）**。大任务我拆给本地或远程（A2A）子 Agent 并行，事件按 key 精确传接 **（子 Agent 编排）**；我的每次 LLM 调用都被记录为轨迹，可直连 AReaL 训练 **（RL 集成）**。
