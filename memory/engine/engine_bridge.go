...
func (b *engineBridge) SearchByEmbedding(query []float32, topK int) ([]memory.EventReference, error) {
	...
	vs, ok := b.engine.(memory.RawVectorSearcher)
	if !ok {
		return b.inner.SearchByEmbedding(query, topK)
	}
	...
	refs := make([]memory.EventReference, 0, len(events))
...
func (b *engineBridge) QueryEvents(query memory.QueryOptions) ([]memory.EventReference, error) {
...
// 截断由引擎内部 textForIndex（MaxTextRunes）处理，此处不截断（不动 memory.FullEvent.Content）。
func textForIndex(event memory.FullEvent) string {
	if event.Content != "" {
