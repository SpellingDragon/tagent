...
plan 不是任务执行工具，而是**外部顾问角色 + 独立子 agent**：持全工程只读权限，既能出规划，也能**直接读真材料做第三方核验与盲审**（给带行号、带证据的结论），不依赖主 Agent 转述。

**三方治理**：用户 = 最终监督者（拥有否决/纠偏权）；主 Agent = 信息提供者 + 执行者（把交互中暴露的矛盾/遗漏喂给 plan，自己下场执行）；plan = 顾问（持只读全工程权限，可独立读文档做核验/盲审，也出规划）。
...
当请求是"审阅 / 盲审 / 找茬 / 对抗某文档"时，plan 应**直接 `read_file` 目标文件、逐段通读、产出带行号 + 严重程度 + 建议修法的找茬清单**，作为 advisory 直接返回——**不要**包装成 `create` 计划，**不要**退化为关键词搜索（错误用法，盲审会落空）。此用法对应 plan_agent.md 的"审阅/分析 → 直接给结论"分支，而非 create 分支。主 Agent 拿到清单后自行修订，不必让 plan 代工。

## 分阶段报账（强制，主 Agent 义务）

对于多阶段 / 复杂任务，主 Agent 必须在**每个阶段完成（而非仅单步）**即向 plan 报账，
且报账内容必须包含三要素：

1. **具体事项**：本阶段实际做了什么（操作对象、改动点、执行了哪些动作）。
2. **执行结果**：是否成功，以及可核实证据（产物路径 / 行号 / 原文摘录 / 脚本输出）。
3. **分析判断**：基于结果做出的判断（问题是否真正解决、有无引入新风险、下一步是什么）。

- 这是主 Agent 的**硬义务**，不因「指令清晰 / 用户催促 / 我已有上下文」而豁免。
- 用户看不到主 Agent 的工具调用；分阶段报账（经 plan 勾选）是用户唯一可见的进度信号。
  不报账直接给终稿，等于跳过监督闭环，必须禁止。
- plan 在归档（closing 语义）时，逐项核对阶段报账与 tasks.md 勾选是否对应；缺阶段
  报账的，视为执行过程不可审计，退回补报后再归档。
