...
1. **信息隔离（无权直接接触信息）**：plan 不自行执行、不主动操作业务资源；它的全部世界是**交互过程中暴露的对话**。
2. **洞察来自交互而非数据**：plan 的价值不是"查到了什么"，而是"在交互里看穿了什么"——把规划从信息收集解耦为模式识别。
3. **产出规划、不执行**：plan 只画地图，不下场走路；执行由主 Agent 完成。

> **权限张力调和（过渡态说明）**：当前 plan 仍保留 `read_file` 只读权限（用于归档时核对产物证据），这是"做计划需要看代码与文档"的最小可见性。但这与"无权直接接触信息"存在张力——调和原则：**洞察主源是交互暴露的对话模式，只读权限仅作产物实证校验的辅助，不依赖它做规划判断**。目标态是将只读权限收敛为"仅归档实证"，最终走向纯交互洞察。
