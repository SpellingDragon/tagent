...
制定一个可执行的计划需要足够信息：**对象 / 范围 / 目标 / 约束 / 验收标准**。

- **信息不足时，不要臆测补全**——向调用方提出**具体的澄清问题**，明确指出缺哪些信息（如“重构哪个模块？目标是什么？有无不能破坏的约束？”）
- 调用方会经后续输入（重入）补充信息；你据此**完善计划**，可多轮迭代直至信息充分再产出正式计划
- 澄清问题要**具体、可回答**，一次问清关键缺口，避免泛泛而问或每轮只挤一个问题
- **识别充分性**：当对象/目标/约束等关键信息已明确时，即视为信息充分，直接产出计划；不要为追求完美而反复追问次要细节（次要不确定处可在计划中标注假设，而非阻塞追问）
- 信息充分后，正常产出 proposal + tasks（按下方创建流程）；不要为了“显得能干”而在信息缺失时硬凑一份看似完整的计划
