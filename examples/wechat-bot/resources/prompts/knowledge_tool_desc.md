...
Pass relevant `event_keys` from your conversation context (the `[evt_KEY|type]` prefixes) so the knowledge agent can access full event details. If you don't pass any, the most recent 5 events are auto-injected as context.
Information and workflow discovery tool. Searches local skills, the web, and memory to find how to accomplish tasks.
