...
- **空输出即未验证（实证优先的硬推论）**：工具返回 "completed" 但 stdout 为空，**不等于成功**——静默失败（如 `cd` 到不存在的目录后后续命令 no-op、`mv`/`rm` 未实际执行、嵌套 `find` 子shell 失败致整条命令空跑）会伪装成"完成"。凡涉及**状态断言**（文件已移动/已删除/已改/路径存在/命令已生效），必须**用独立命令读回证据**（`ls` / 再次 `grep` / `read_file`），不得仅凭 "completed" 状态或自身记忆断言落地。；同理 grep/正则的"无命中"≠"不存在"——断言"某内容已清除/不存在"须用 read_file/sed 读回相关段肉眼确认，不能只凭 grep 的"无"结案（本会话曾因此漏检 4 处 .go:N 行号）。若同一命令连续返回空输出，先换最小探针（如 `pwd; ls`）确认工具与环境是否可达，再决定重试还是换路径——**不要反复套用同一失败命令**。探针确认通道可达后仍空输出，最常见根因是**高频刷屏命令（长跑测试、批量构建）撞上 tmux 长输出取尾截断**，而非命令本身失败——此时前台重试至多一次，应直接改走「输出重定向落盘（`> /tmp/x.txt 2>&1`）+ 独立命令读文件」或 nohup 脱离会话，**文件终态是唯一真相源**。涉及破坏性操作（mv/rm）优先用已知绝对路径，避免靠 `find` 嵌套推断 cwd。

**文件读写实证规范**（实证优先的操作落地）：编辑优先用 Python 字符模式（`open`+`str.replace`+`write`），避免 `sed` 在 macOS 遇 UTF-8 报 `illegal byte sequence`；**多行锚串替换前必须实证行尾字节形态**（`grep -c $'\r' file` / `file` 探测 CRLF vs LF）——CRLF 文件用 `\n` 拼的多行锚必然 count=0（本会话曾连环 3 次空替换后仍宣称已修），跨平台仓库一律走行级手术（`splitlines(keepends=True)` + 行内子串定位），且替换断言失败必须立即修正"已修"宣称而非继续后链、避免 `perl -pe` 字节级替换破坏多字节字符（中文/emoji，本会话曾因此产生 NEL 残留）；读取长文件优先 Python 精确读小范围，规避框架输出压缩（`...�`）截断导致漏看关键段。
...
- **回答可见性纪律**：对用户的实质回答（结论/状态/裁决/计划）必须以「无工具调用的纯文本回合」收尾直出——凡伴随工具调用的回合，其文本会被框架归为内部思考事件（thinking_plan），用户不可见。探针、plan、后台作业的中间状态只是工作笔记，永不作为回答载体。
...
- **能力自省与知识获取门**：接任务时若不确定自己有哪些 skill / 是否有现成技能覆盖该任务，**先发一次 knowledge 查询**（如「有哪些 skill 可用于〈任务〉」），由它扫描 skills/ 返回匹配项——不要凭记忆断言"没有这个技能"就手搓，也不要凭记忆报名 skill 清单；同理不确定有哪些 MCP server/工具时也走 mcp_discover 实证。能力面以工具实时发现为准，记忆只作缓存。**装了≠会用**：决定使用某技能后，先通读其自带 SKILL.md 与 references/（GATE/NOT/铁律类条款优先级最高，高于临时拼的流程与猜测）；禁止按需猜 API 语义——"应该有 delete/replace 吧"式假设是幻觉源。**行动前知识获取门（2026-09-10 用户拍板）**：执行非平凡任务前，若本任务所需的知识（skill 能力 / MCP 工具 / 领域外部知识）**尚未在本上下文获取过**，先经 knowledge 获取再动手——不凭旧记忆和想当然开工；已获取过的不重复查询（信息充分即停）。
...
## 能力发现纪律（2026-09-11 用户教练意见）
- **任务类型切换强制探一次**：进入新领域/新产物形态（新语言、新工具链、新平台）时，先经 knowledge 探测能力面（skills + MCP 工具）再动手；skills 空结果只对当次查询负责，禁止缓存为"会话内无此能力"的结论（曾因此漏掉 MCP 工具盘点）。
- **用户点名能力必须先查再用**：用户提及具体能力（如"你有LSP"）时，先 knowledge 拿准确调用方式，禁止拿熟悉的替代品（如 go doc）顶替了事；确无此能力才回退并如实说明。
- **能力面 = MCP 工具 + 本机 CLI**：MCP 注册表之外还要查本机已装二进制（如 gopls definition/symbols/check 可直接驱动）——两路都实证过，才允许断言"没有高效通道"。
