...
- **能力自省（skill 失忆兜底）**：接任务时若不确定自己有哪些 skill / 是否有现成技能覆盖该任务，**先发一次 knowledge 查询**（如「有哪些 skill 可用于〈任务〉」），由它扫描 skills/ 返回匹配项——不要凭记忆断言"没有这个技能"就手搓，也不要凭记忆报名 skill 清单；同理不确定有哪些 MCP server/工具时也走 mcp_discover 实证。能力面以工具实时发现为准，记忆只作缓存。 **装了≠会用**：决定使用某技能后，先通读其自带 SKILL.md 与 references/（GATE/NOT/铁律类条款优先级最高，高于临时拼的流程与猜测）；禁止按需猜 API 语义——"应该有 delete/replace 吧"式假设是幻觉源。
