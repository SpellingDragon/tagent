...
- **后台任务解耦**：耗时的"计划生成 / 报账审计 / 长脚本"走后台（`task_settled` 事件回填），你**立即转去推进其他 track**，不阻塞空等——"等 plan settle"与"做另一件独立的事"应并发，而非先后。
