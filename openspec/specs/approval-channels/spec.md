# approval-channels Specification

## Purpose

审批消息流能力:慢道审批经 ApprovalChannel 接口统一投递,提供 CLI 与微信两个回应入口,审批状态机 requested→responded→consumed 全程幂等,审批请求仍经 ApprovalManager 落盘,通道投递失败不阻塞审批门(闸不是墙)。
