# degradation-behaviors Specification

## Purpose

依赖退化行为响应能力:DegradationManager 启用时对 model/mcp/disk 三类依赖退化提供独立可配置的警告级行为响应(turn 间退避、mcp_call 熔断快失败加半开探测、disk 退化拒绝新 spawn),每项默认关闭(零行为变化),依赖恢复即自动回正常路径。
