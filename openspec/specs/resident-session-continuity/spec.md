# resident-session-continuity Specification

## Purpose

常驻/交互 tmux 会话的跨重启连续：元数据完备（command/task_id/origin）入事实链、会话枚举修复与 orphan 语义重定义（cleanup 排除 n-）、存活探测三态化与 fail-dead 加闸、唯一挂载点重挂与 TaskID 桥任务重关联。与 task-registry-and-board（R2）联动供能跨重启 resume。
