...
规定 tagent 变更（openspec change）的执行治理规则：阶段依赖与准入条件、准出三道门禁与自修轮次上限、
真实 LLM 测试失败的定性义务、design 对四项全局不变量的声明义务、sub-agent 分工惯例、以及归档时的
delta specs 同步与父级路线图状态回写义务。

源自 `tagent-evolution-roadmap`（P0-P4 多阶段马拉松）的父路线图治理契约。该路线图已于归档日以
「部分完成」形态归档（成果清单与终止点见其归档 tasks.md §6.3），但上述六条规则对**后续任何多阶段
变更集**仍具约束力，故随归档同步为主规格保留——遵循 LEDGER 重编原则 #1「裁决集中记录、不因归档丢失」。
