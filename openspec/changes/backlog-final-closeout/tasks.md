...
- [ ] 1.3 R3:溢出落盘后经 persistBusEvent 登记 overflow_dump 事件(Summary 含路径+大小;尽力而为失败仅日志);回归:溢出后 recall 可命中票据
- [ ] 1.4 R4:platform wiki reliability 节注记 hintTracker 会话态(一行)
- [ ] 1.5 R5:example 装配 wechatApprovalChannel(实现 Deliver→SendTextToUser)+README 补直投说明;回归:Deliver 调用消息送达(mock bot)

## 2. evals 骨架(D4 精简)

- [ ] 2.1 evals/ 目录+README+evals_test.go runner(go test ./evals/ 即跑)
- [ ] 2.2 票据可召回率 suite:构造→压缩→逐 key recall 断言原文(mock 进 CI);回归:全量取回
- [ ] 2.3 工具选择 suite+Bad Case 资产(hex 断裂/静默空结果两用例转 cases/);回归:Bad Case 断言显式拒绝
...
- [ ] 3.3 M4:HTTPAPI `GET /feedback/wait` long-poll(队列+30s 上限)+TurnTracker(TypeTurnStart 计数入 Evidence.TurnCount);回归:写入 feedback 后 wait 立即返回;TurnCount 非零
