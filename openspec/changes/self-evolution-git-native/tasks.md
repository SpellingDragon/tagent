...
- [ ] 3.2 `tagent.go` 接线:evolution 启用时注册新 refine 工具(entry only,先于治理包裹——A3 不变);classifier refine 规则核对(动词更新)
...
- [ ] 4.4 feedback 章迁移:`bundleIDFn` 来源改**最新 improvement 事件的 sha**(事件查询,无组件);无事件不盖章;`TestBindFeedback_InheritsBundleID` 等既有 join 测试保持绿(8.4 语义不变)
