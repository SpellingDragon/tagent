...
| T-G | COMPLETE | REG,FIX(done) | F2/F3已缓解;门禁3揪出**Blocker(事件时序倒置→回复路由错误会话)**+Major(approval竞争/critical绕过/judge缺score误回滚/canary ctx假通过/eval Limit静默失效/reclaim瞬时删/无背压)全修+回归锁定 | ✅governance(RiskClassifier C5+Budget滑窗epoch持久+Approval异步digest绑定+DenialLedger治理事件+Goal+Gate管线+GovernanceTool装饰器LIVE+trigger source盖章goal-required+账本绑定memStore持久化)+reliability(**DegradationManager五依赖LIVE**:ErrorTrackingStore C2最外层挂点[报告D3设计,此前缺失致零接线]+MemorySink依赖倒置+event_loop model上报+mem_spill退化兜底重放[at-least-once延伸存储层];4/5依赖接线memory/disk/rustviking/model,mcp待专项+**ReliableBus全序磁盘溢出**:channel恒早于spill/pending背压上限/批量上限/瞬时错误重试不删/重启恢复+**AnchorStore冥想锚点持久化**anchorMu一致快照)+故障注入矩阵(多依赖独立退化/时钟回拨/并发压力/坏文件);配置门控默认关闭零行为变化 |
