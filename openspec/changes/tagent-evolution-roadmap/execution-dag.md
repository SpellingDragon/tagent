...
- **archive 执行完成(2026-09-06,/opsx:archive「整合合并归档」)**:ready-with-notes 三步路径已贯通——
  - **补 proposal.md**:postmerge-review-fixes/proposal.md 落地(来源=§8 审计修复容器;声明**无 spec delta**=纯缺陷修复,归档走 `--skip-specs`);openspec status proposal 由 ready→**done**。
  - **新发现(超 §8.11 Minor 清单)**:`check-openspec.sh` 原**FAIL**——4 主 spec 非 strict-valid,系 hybrid(7.2)+obs(6.2)同步遗留:`semantic-search`/`recall-hybrid-fusion`/`turn-tracing` 缺 `## Purpose`,`trajectory-trace-correlation` 尾部残留 `## MODIFIED Requirements`(delta 格式泄漏进主 spec)。**整合修复**:4 spec 补 `## Purpose`、trajectory 的 `## MODIFIED` 段并入 `## Requirements`(信息注释保留);修后 `check-openspec.sh` **75 passed / 0 failed**(主 specs 全 strict-valid)。
  - **归档**:`openspec archive postmerge-review-fixes --skip-specs -y` 成功→`openspec/changes/archive/2026-09-06-postmerge-review-fixes/`(--skip-specs 绕过"无 delta"校验,无需 --no-validate;目录 .openspec.yaml+proposal.md+tasks.md 完整迁移);openspec list 余 3 活跃变更。
  - **5.3 处置**:遵 LEDGER 重编原则#1「归档区不改写」,**不改动已冻结的归档 tasks.md**(5.3 维持 `[ ]`=archive 动作自身,归档命令已记 1 incomplete warning 经 --yes 续行);5.3 闭环由本记录 + LEDGER 权威账本承载。postmerge-review-fixes 正式收官。
...
- **裁定:N1/N2 均真修且质量过关**——N1(releases.jsonl 追加式持久化[submitMu 保护无并发写,append 为正确取舍]+ 构造期 loadHistory 重放[坏行跳过]+ NoteActive 基线 seed[Active()==nil 门控防重启重复];seed 链路闭环:白名单只认 jsonl、active.json 只管指向);N2(rc.govLedger **单实例**共享全部 gate;BindStore 延迟绑定时序正确——子 agent gate 先建于 entry 工具递归期、Record 仅运行期发生、绑前内存缓冲+rebuild 不清空;测试覆盖三态)。Minor 8 抽验:①③④⑤⑥⑦全 PASS(预算 Dir 守卫/prev-active 回退/审批过期清理判据正确/宣称校准诚实/工厂边界标注相符/注释固化相符)。
- **新遗留 1 Major**:**N2 治理事件无来源 agent 归属**——DenialRecord 与事件 metadata 均无 agent 字段,多子 agent(knowledge/action/自定义)审计查询无法区分来源,牺牲 N2 本要建立的审计价值(ledger.go:114-136 + tagent.go:564)。修法:DenialRecord 加 AgentName + GateDeps 注入 agent name + writeGovernanceEvent 写 metadata["agent"]。
- **1 项宣称不符**:tasks 7.2 勾选的"buildAgent 级子 agent exec 过闸测试"不存在(仓内仅 governance 包模拟双 gate 测试;wire 测试只断言 New 成功)——补双 agent+exec 真实构建过闸测试或改回措辞。
- **Minor 8**:①Ledger.Record 无锁读 store/partitionID(启动期 happens-before 免疫,锁纪律不一致);②N1 缺 Submit→重载→refineRollback 成功用例;③N2 buildAgent 时序无集成测试(gate 同指针断言);④InitBaseline 崩溃窗口(active.json 已写而 releases.jsonl 未写→基线永久不入白名单;NoteActive 幂等化或重启补 seed);⑤BindLedger 死代码且语义与 BindStore 相反(建议删);⑥slow 道 SetActive 前 active==nil 守卫(InitBaseline 失败被 Warn 吞+refine 照注册→拒绝分支回滚落空);⑦W2 节流测试真实时间依赖(CI 重载>2s 假失败风险,注入时钟);⑧hybrid tasks 头部过时 BLOCKED 措辞。
- 门禁亲验:build/vet ✅ + 全量 -short **39 包零 FAIL** + evolution/governance/reliability/memory/event/rl/**tool 全域** -race 全绿;工作区亲验干净。
- **收尾状态**:postmerge-review-fixes 实质完成(6 Major+N1/N2+Minor 20/23;M1/M4/M7 标注后续);启用禁令全解除成立;**5.3 archive 仅差维护者裁决**(选项:补审计归属后 archive[推荐,小时级] / 降级标注直接 archive;Jaeger/AReaL BLOCKED 与代码无关属环境实装)。第四轮遗留已增补至 tasks §8/§9,**§8 关闭前 5.3 不得执行**。
- **第四轮遗留修复完成(2026-09-06,第五轮;§8/§9 全部关闭)**:
  - §8.1 Major(N2 治理事件来源归属)**FIXED**:DenialRecord.AgentName(json `agent,omitempty`)+ GateDeps.AgentName + GovernanceGate.agentName + writeGovernanceEvent metadata["agent"](omitempty,单 entry 不写噪声键)+ rebuildFromStore 回读 + tagent.go:565 接线传 `name`。
  - §8.2(宣称不符)**FIXED**:新增 governance_wire_test.go `TestBuildAgent_GovernanceWrapsAllAgents_SharedLedger`——走真实 buildAgent 构建 entry(tagent)+ 子 agent(worker),经 exec 工具链(OutputLimitTool→GovernanceTool→mock,Declaration.Name="exec")驱动 critical `rm -rf`,断言两 agent 均被拒(`[governance_denied]`)且记录按 AgentName 区分来源;fail-before 探针(临时置 AgentName="")验证 sawEntry/sawSub 断言双双失败。
  - Minor 8 全 **FIXED**:①Ledger.Record 锁内快照 store/pid(消与 BindStore 的 -race 竞争);②n1_test `TestRelease_SubmitReloadRefineRollback`(Submit 快道 active→重载 loadHistory→refineRollback 曾 active draft 成功);③governance_wire_test 断言两 agent 记录落同一 rc.govLedger(行为证明同指针);④release.go `seedActiveBaseline`(NewReleaseManager 对当前 active 补 seed)+ NoteActive 幂等,修 InitBaseline 崩溃窗口;⑤删 gate.go `BindLedger` 死代码(删后不再 import memory);⑥Submit `active==nil` 直接 reject 守卫(fail-before 探针:禁用守卫则孤儿 draft stage=active 滞留);⑦approval.go 注入 `now func()`+`rescanInterval`,假时钟确定性验节流 + 补 `TestApproval_RescanWindowExpiryResumes` 窗过期恢复正向用例;⑧hybrid-semantic-recall/tasks.md 头部 + 7.3 过时 BLOCKED 措辞同步(5.1/5.3 已 2026-09-06 真实 key 实测完成)。
  - 门禁复验:gofmt/build/vet ✅ + 全量 -short 零 FAIL + governance/evolution/root -race 全绿。postmerge-review-fixes §8/§9 清零,**5.3 archive 阻断解除**(仅差维护者裁决 archive 时机)。
