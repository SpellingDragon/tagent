...
| T-G | IN_PROGRESS(原语done,gate-3修) | REG,FIX(done) | F2/F3已缓解(核验非缺陷);门禁3揪出approval数据竞争已修(锁内快照) | ✅governance(RiskClassifier C5+Budget滑窗epoch持久+Approval异步digest绑定+DenialLedger治理事件+Goal+GovernanceGate管线)+reliability(DegradationManager五依赖状态机);余(集成层):GovernanceTool装饰器接线agent.go+ReliableBus(spill/durable)+AnchorStore+故障注入矩阵+config |
