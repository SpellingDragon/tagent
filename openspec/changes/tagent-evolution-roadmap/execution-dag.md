...
| T-G | IN_PROGRESS(原语done) | REG,FIX(done) | F2/F3已缓解(核验非缺陷);governance+reliability原语-race绿 | ✅governance(RiskClassifier C5+Budget滑窗epoch持久+Approval异步digest绑定+DenialLedger治理事件+Goal+GovernanceGate管线)+reliability(DegradationManager五依赖状态机);余(集成层):GovernanceTool装饰器接线agent.go+ReliableBus(spill/durable)+AnchorStore+故障注入矩阵 |
...
| T-EVO | IN_PROGRESS(核心done) | TC0,TG(C5),TB,FIX,REG(done) | 风险分级发布道调和T-E/T-F:快道(validate→canary→后验评估→通过active/劣化回滚)+慢道(+replay/shadow/approve门后生效);双回滚(guardrail确定性+evaluator模型);protected强制慢道;agent无直接激活权(D1铁律) | ✅ReleaseManager(evolution/release.go)7测试-race绿,复用TC0 BundleStore+T-G RiskRouter(C5);余:refine工具(propose/diff/status/rollback无activate)+Evaluator实现(LLM-judge)+BundleProvider接线归因bundle_id |
