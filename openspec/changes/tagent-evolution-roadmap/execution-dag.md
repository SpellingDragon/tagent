...
| T-EVO | IN_PROGRESS(核心+refine done) | TC0,TG(C5),TB,FIX,REG(done) | 风险分级发布道调和T-E/T-F;门禁3 CodeReview揪出Blocker(评估器error假激活)已修+回归锁定 | ✅ReleaseManager(快道后验/慢道门后+双回滚+protected强制慢道+agent无直接激活权)+refine工具(propose/diff/status/rollback无activate);gate-3修复:评估器error保守回滚/Submit序列化/bundle id路径遍历守卫;余:Evaluator实现(LLM-judge)+BundleProvider接线归因bundle_id+config |
