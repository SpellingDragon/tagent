...
| T-G | IN_PROGRESS(原语+接线done) | REG,FIX(done) | F2/F3已缓解;门禁3揪出approval数据竞争+critical无批准绕过均已修 | ✅governance(RiskClassifier C5+Budget+Approval+DenialLedger+Goal+Gate管线+**GovernanceTool装饰器接线entry leaf工具LIVE**)+reliability(DegradationManager五依赖状态机);配置门控默认关闭零行为变化+接线测试绿;余:ReliableBus(spill/durable)+AnchorStore+故障注入矩阵+治理事件持久化接memStore+event loop盖章trigger source |
...
| T-EVO | IN_PROGRESS(核心+refine+router+接线LIVE) | TC0,TG(C5),TB,FIX,REG(done) | 风险分级发布道调和T-E/T-F;门禁3揪出Blocker(评估器error假激活)+路径遍历已修+回归锁定;DiffRiskRouter接线使快慢道按diff实际路由 | ✅ReleaseManager(快道后验/慢道门后+双回滚+protected强制慢道+agent无直接激活权)+refine工具+DiffRiskRouter(模型/参数→慢道,仅提示词→快道)+**运行时接线LIVE**(New构造BundleStore/ReleaseManager,entry agent VersionedSource读bundle+refine注册,配置门控);余:Evaluator实现(LLM-judge)+Guardrail(指标闸)+归因bundle_id+治理事件持久化 |
