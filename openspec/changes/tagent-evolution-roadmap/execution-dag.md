...
| T-G | IN_PROGRESS(governance完整LIVE+reliability核心) | REG,FIX(done) | F2/F3已缓解;门禁3揪出approval竞争+critical绕过已修;trigger source盖章使goal-required LIVE | ✅governance(RiskClassifier C5+Budget+Approval+DenialLedger+Goal+Gate管线+GovernanceTool装饰器LIVE+trigger source盖章+账本绑定memStore持久化)+reliability(DegradationManager五依赖状态机+**ReliableBus:SpillStore磁盘溢出/channel满不丢事件at-least-once/Pull优先回收/重启恢复seq/per-agent子目录配置门控**);余:AnchorStore(冥想锚点持久化)+故障注入矩阵 |
